...
myConfHook (pkg0, pbi) flags = do
    ...
    let libbi' = libbi
          ...
          , cxxOptions    = cxxOptions    libbi ++ cxxOptions    wx ++ ["-DwxcREFUSE_MEDIACTRL"]
          }
...
generateHeaders wxVersion =
  do
    writeFile ("include" </> "wxc" </> "wxc_def.h")
  ...
  where
    ...
    f b w =
        case w of
          ...
          ('-':'D':_) -> b { cxxOptions    = w : cxxOptions b }
          _           -> b
...
myBuildHook pkg_descr local_bld_info user_hooks bld_flags =
    ...
    objs <- mapM (compileCxx gcc cxx_opts inc_dirs bld_dir) dll_srcs
